...
from libs.utils import convert, now_dir, data_path, get_data_list
from libs.fb2_processor import FB2Processor
from libs.tts import synth, get_model_short_name
...
from libs.ui_assets import format_audio_time, format_time_hms, get_metrics_html, get_batch_metrics_html, get_batch_summary_html
...
def tts(ab_path, repl, spk_sel, sp_rate, back_sound_sel, bitrate, noise_lvl, use_sound_effect, use_accents):
    ...
    if total_lines == 0:
        yield get_files_list(ab_path), "⚠️ Нет строк для озвучки!", get_metrics_html(0, "00:00", "00:00", "0.0")
        return
    ...
    if times_file.exists():
        try:
            with open(times_file, 'r', encoding='utf-8') as f: parse_times = json.load(f)
        except: pass
    ...
    yield cached_files_list, "⏳ Инициализация движка...", get_metrics_html(0, "00:00", "Оценка...", "0.0")
    ...
    def safe_synth(t, disable_norm=False):
        ...
        if use_accents and not disable_norm:
            try: txt = accentizer.process_accent(txt, r'\+\w+|\w+\+\w+')
            except Exception: pass
        res = synth.synth_audio(txt, speaker_id=spk_sel, speed=sp_rate, noise=noise_lvl)
        if res is None: return None, None
        a, b = res
        if isinstance(a, (int, float)): sr, aud_raw = int(a), b
        elif isinstance(b, (int, float)): sr, aud_raw = int(b), a
        else: sr, aud_raw = 24000, a
        return sr, aud_raw
    ...
    def _tts_to_audio(np_audio, sr):
        ...
        if np_audio is not None and getattr(np_audio, 'size', 0) > 0:
            ...
            audio = AudioSegment(final_audio_int16.tobytes(), frame_rate=sr, sample_width=2, channels=1)
    ...
    for file in sorted(files, key=safe_sort):
        ...
        if mp3_file.exists() and not repl:
            x_file = xml_path / f'{file}.xml'
            try: current_line += len(etree.parse(str(x_file)).getroot())
            except: pass
            yield cached_files_list, f"⏭ Пропуск: {short_final}.mp3 уже существует", get_metrics_html(int((current_line/total_lines)*100), format_time_hms(time.time()-global_start_time), "00:00", "-")
            continue
        ...
        audio_segments = [AudioSegment.silent(duration=1000, frame_rate=24000)]  # Начальная пауза 1 сек + накопление сегментов
        
        # ── БУФЕР ДЛЯ БАТЧИНГА EDGE TTS ──
        text_buffer = []
        ...
        for i, line in enumerate(root):
            ...
            if current_line % 3 == 0 or current_line == total_lines:
                ...
                html = get_metrics_html(pct, format_time_hms(elapsed), format_time_hms(rem_sec), f"{speed:.1f} стр/с")
            ...
            if line.tag == 'sound' and use_sound_effect:
            ...
            elif line.tag in ('cite', 'empty-line') and use_sound_effect and not line.text:
                ...
                text_buffer.append(line.text.strip())
            ...
            if line.tag == 'cite' and use_sound_effect:
                ...
                if line.get('position') == 'start':
                    pr_audio = AudioSegment.from_wav(sound_dir /'pause' / 'min_cite.wav')
        ...
        if back_sound_sel:
            ...
            if back_duration_ms <= clip_length_ms: st_poz = 0
            else: max_start = back_duration_ms - clip_length_ms; st_poz = random.randint(0, max_start)
        ...
        tags = {'artist': autor, 'title': f'{short_final}', 'track': f'{file}', 'album': album}
        
        # Определяем имя файла (полное или частичное)
        save_path = str(partial_mp3_file) if was_interrupted else str(mp3_file)
        
        if cover_file.exists(): out_audio.export(save_path, format='mp3', bitrate=f'{bitrate}', cover=str(cover_file), tags=tags)
        else: out_audio.export(save_path, format='mp3', bitrate=f'{bitrate}', tags=tags)
        ...
        with open(times_file, 'w', encoding='utf-8') as f: json.dump(parse_times, f)

        if was_interrupted:
            pct = int((current_line / total_lines) * 100) if total_lines > 0 else 0
            yield get_files_list(ab_path), f"🛑 Остановлено! Частичный файл сохранен как {short_final}_PARTIAL.mp3", get_metrics_html(pct, format_time_hms(time.time()-global_start_time), "-", "Остановлено")
        ...
        yield cached_files_list, f"✅ Сохранен: {short_final}.mp3", get_metrics_html(int((current_line/total_lines)*100), format_time_hms(time.time()-global_start_time), format_time_hms((total_lines-current_line)/(current_line/(time.time()-global_start_time))), "-")
    
    # ФИНАЛ
    yield cached_files_list, "🎉 ВСЕ ФАЙЛЫ УСПЕШНО ОЗВУЧЕНЫ!", get_metrics_html(100, format_time_hms(time.time() - global_start_time), "00:00", "-")
...
def get_files_list(ab_name):
    ...
    if d_path.exists():
        ...
        def safe_sort(file_path):
            stem = file_path.stem
            try: return (0, float(stem))
            except ValueError: return (1, stem.lower())
        ...
        for file_path in sorted_files:
            ...
            file_url = f"/gradio_api/file={data_path}/{ab_name}/mp3/{file_path.name}?t={mtime}"
            # Компактный аудиоплеер
            audio_html = f'''
                <audio controls preload="none" style="width:180px;height:26px;">
                    <source src="{file_url}" type="audio/mpeg">
                </audio>
            '''.strip()
            # Кликабельное имя файла: скачать по клику + кнопка игры
            filename_html = f'''
                <div style="display:flex; align-items:center; gap:6px; white-space:nowrap;" data-filename="{file_path.name}">
                    <a href="{file_url}" download="{file_path.name}" 
                       style="color:#38bdf8; text-decoration:none; font-weight:500; font-size:12px;"
                       title="💾 Скачать {file_path.name}"
                       onclick="event.stopPropagation();">
                        📥 {file_path.name}
                    </a>
                    <a href="{file_url}" target="_blank"
                       style="color:#10b981; text-decoration:none; font-size:14px; line-height:1;"
                       title="▶ Играть"
                       onclick="event.stopPropagation();">
                        ▶
                    </a>
            ...
            rows.append([audio_html, filename_html, model_label, f"{size_in_mb}M", format_audio_time(float(info['duration'])), ptime_str])
    return rows

def create_zip_archive(ab_path):
    mp3_dir = data_path / ab_path / 'mp3'
    if not mp3_dir.exists(): raise gr.Error(f"Папка с файлами не найдена!")
    
    with tempfile.NamedTemporaryFile(suffix='.zip', delete=False) as tmp: zip_filename = tmp.name
    try:
        ...
        if Path(zip_filename).exists(): Path(zip_filename).unlink()
        raise gr.Error(f"Ошибка: {str(e)}")

def snd_list():
    snd_path = sound_dir / 'back'
    if snd_path.exists(): return gr.update(value = '', choices = sorted([x.name for x in snd_path.iterdir()]))
    return gr.update(value = '', choices = [])

def del_file(filename, ab_name):
    if not filename: return get_files_list(ab_name)
        ...
        try:
            ...
            if file_path.stem in model_map:
                ...
                try:
                    ...
                    pass
        except Exception as e: gr.Warning(f"Ошибка удаления: {e}")
    return get_files_list(ab_name)

def sel_file(data: gr.SelectData, ab_path):
    mp3_dir = data_path / ab_path / 'mp3'
    filename_html = data.row_value[1]
    # Извлекаем имя файла из data-filename атрибута
    match = re.search(r'data-filename="([^"]+)"', filename_html)
    if match:
        filename_raw = match.group(1)
    else:
        # Fallback: ищем текст после иконки 📥
        text_match = re.search(r'📥\s*(\S+)', filename_html)
        if text_match:
            filename_raw = text_match.group(1)
        else:
    ...
    return gr.update(interactive = True), str(selected_path), gr.update(visible=True), gr.update(value=stem_name)

def rename_selected_file(current_path, new_name, ab_path):
    if not current_path or not new_name: return get_files_list(ab_path), gr.update(visible=False)
    ...
    if not safe_name: safe_name = "renamed_audio"
    ...
    try:
        ...
        if old_path.stem in model_map:
            ...
            try:
                ...
                pass
    except Exception as e: gr.Warning(f"Ошибка переименования: {e}")
    return get_files_list(ab_path), gr.update(visible=False)
...
def batch_tts_all_projects(spk_sel, sp_rate, back_sound_sel, bitrate, noise_lvl, use_sound_effect, use_accents, repl, selected_projects=None):
    ...
    if not all_projects:
        yield [], "⚠️ Нет проектов для пакетной озвучки!", get_batch_metrics_html("", 0, 0, 0, 0, "00:00", "00:00", "0.0")
        return
    ...
    yield [], f"📦 Подготовка {total} проектов...", get_batch_metrics_html("Подготовка...", 0, 0, total, 0, "00:00", "...", "...")
    ...
    for idx, project in enumerate(all_projects, 1):
        if stop_text_to_sp:
            ...
            partial_html = get_batch_metrics_html(project, 0, idx, total, pct, format_time_hms(elapsed), "-", "Остановлено")
            if batch_stats:
                partial_html = get_batch_summary_html(batch_stats) + "\n" + partial_html
            
            yield all_files, f"🛑 Остановлено! Озвучено {processed_count} из {total}", partial_html
            return
        ...
        if not xml_dir.exists() or not list(xml_dir.glob('*.xml')):
            ...
            yield all_files, f"📁 [{idx}/{total}] {project}: Парсинг FB2 → XML...", get_batch_metrics_html(project, 0, idx, total, batch_pre_pct, format_time_hms(elapsed), format_time_hms(rem), f"~{format_time_hms(sec_per_proj)}/пр")
            try:
                # Используем сохранённые настройки парсинга из конфигурации
                proc = FB2Processor()
                for _ in proc.process_book(
                    ab_path=project, replace=repl,
                ...
                if not clean_album: clean_album = project[:20]
                ...
                for xml_file in xml_dir.glob("*.xml"):
                    ...
                    content = re.sub(r'(<(?:speak|root)[^>]*>)[\s\S]*?<p>(.*?)</p>\s*', _clean_head, content, count=1)
                ...
                yield all_files, f"⚠️ [{idx}/{total}] {project}: Ошибка парсинга: {e}", get_batch_metrics_html(project, 0, idx, total, batch_pre_pct, format_time_hms(elapsed), "...", "...")
                continue
        
        # Запуск TTS для проекта
        proj_start = time.time()
        for tts_result in tts(project, repl, spk_sel, sp_rate, back_sound_sel, bitrate, noise_lvl, use_sound_effect, use_accents):
            if stop_text_to_sp:
                break
            ...
            batch_html = get_batch_metrics_html(
                project, project_pct, idx, total, batch_pct,
                format_time_hms(elapsed), format_time_hms(rem_sec),
                speed_str
            )
        ...
        batch_stats.append((project, dur, size_mb, proc_time if proc_time > 0 else time.time() - proj_start))
        processed_count += 1
    ...
    final_bars = get_batch_metrics_html("✅ Завершено", 100, total, total, 100, format_time_hms(elapsed), "00:00", f"~{format_time_hms(sec_per_proj_final)}/пр")
    summary_table = get_batch_summary_html(batch_stats)
    final_html = summary_table + "\n" + final_bars if summary_table else final_bars
    
    yield all_files, f"🎉 ГОТОВО! Озвучено {processed_count} из {total} проектов", final_html

def change_tts_model(mver):
    sp_list = synth.speakers_list()
    speaker = sp_list[0]
    if isinstance(speaker, tuple): speaker = sp_list[0][1]
    
    try:
        fresh_config = AppConfig.load_user_settings()
        saved_spk = getattr(fresh_config, 'spk_sel', '')
        
        if saved_spk and any(saved_spk == s or (isinstance(s, tuple) and saved_spk == s[1]) for s in sp_list):
            speaker = saved_spk
    except Exception:
        pass
...
def tts_tab(ab_path, tts_state):
    with gr.Tab(label="🎧 ОЗВУЧКА", id=2) as tts_tab_ui:
        with gr.Row():
            try:
                ...
                saved_spk = ''
            spk_sel = gr.Dropdown(value=saved_spk, label='Выбрать основной голос', choices=[saved_spk] if saved_spk else [''], interactive=True)
            sp_rate = gr.Slider(0, 3, config.sp_rate, step=0.1, label="Задать скорость", interactive=True)
        with gr.Row():
            back_sound_sel = gr.Dropdown(value=config.back_sound_sel, allow_custom_value=True, label='Выбрать музыку для оглавлений', choices=[''], interactive=True)
            bitrate = gr.Slider(24, 256, config.bitrate, step=2, label="Задать битрейт аудио", interactive=True)
            noise_lvl = gr.Slider(0, 64, config.noise_lvl, step=1, label="Уровень шума(выше-лучше)", interactive=True)
        
        with gr.Row():
            with gr.Column(scale=3):
                with gr.Row():
                    use_sound_effect = gr.Checkbox(label="Аудио эффекты", value=False)
                    repl = gr.Checkbox(label="Перезаписать существующие MP3", value=True)
                    use_accents = gr.Checkbox(label="Проставить ударения (РУ)", value=True)
            with gr.Column(scale=1, min_width=150):
                tts_button = gr.Button("🟢 TTS (Ctrl+Enter)", elem_id="tts_btn", variant="primary", size="sm")
                batch_tts_btn = gr.Button("📦 Пакетная озвучка выбранных", variant="primary", size="sm", elem_id="batch_tts_btn")
        ...
        metrics_panel = gr.HTML(value=get_metrics_html(0, "00:00", "00:00", "0.0 стр/с"))
        with gr.Group(elem_id="log_group"):
            output_log = gr.Textbox(label="Живой лог озвучки", lines=3, max_lines=3, interactive=False, value="В ожидании старта...")
        ...
        df_output = gr.DataFrame(
            ...
            column_widths=['190px', '180px', '90px', '70px', '80px', '80px'],
            type='array',
            wrap=True,
        )
        ...
        with gr.Group():
            with gr.Row(visible=False) as rename_panel:
                ...
                rename_btn = gr.Button("✏️ Переименовать", scale=1, variant="secondary", elem_id="rename_file_btn")
            
            with gr.Row():
                del_btn = gr.Button("❌ Удалить файл (Delete)", interactive=False, elem_id="del_file_btn", size="sm")
                create_arh_btn = gr.Button("📦 Создать архив", size="sm")
                download_btn = gr.DownloadButton("📥 Скачать архив", value=None, variant="primary", visible=False, size="sm")

    download_btn.click(fn=lambda: gr.DownloadButton(visible=False), outputs=download_btn)
    create_arh_btn.click(create_zip_archive, inputs=ab_path, outputs=download_btn)
    df_output.select(sel_file, inputs=ab_path, outputs=[del_btn, cur_file, rename_panel, new_name_input])
    rename_btn.click(rename_selected_file, inputs=[cur_file, new_name_input, ab_path], outputs=[df_output, rename_panel])
    
    tts_button.click(
        fn=lambda: (gr.update(value="⏳ Подготовка файлов..."), get_metrics_html(0, "00:00", "00:00", "0.0 стр/с")), 
        outputs=[output_log, metrics_panel]
    ).then(
        fn=tts,
        inputs=[ab_path, repl, spk_sel, sp_rate, back_sound_sel, bitrate, noise_lvl, use_sound_effect, use_accents],
    ...
    tts_tab_ui.select(fn=refresh_project_list, outputs=batch_project_sel).then(snd_list, outputs=back_sound_sel)
    
    batch_tts_btn.click(
        fn=lambda: (gr.update(value="📦 Запуск пакетной озвучки..."), get_batch_metrics_html("Подготовка...", 0, 0, 0, 0, "00:00", "...", "0.0")), 
        outputs=[output_log, metrics_panel]
    ).then(
        fn=batch_tts_all_projects,
        inputs=[spk_sel, sp_rate, back_sound_sel, bitrate, noise_lvl, use_sound_effect, use_accents, repl, batch_project_sel],
        outputs=[df_output, output_log, metrics_panel]
    )
    
    stop_btn.click(stop_tts, outputs=output_log, queue=False)
    del_btn.click(del_file, inputs=[cur_file, ab_path], outputs=[df_output]).then(fn=lambda: gr.update(visible=False), outputs=[rename_panel])
